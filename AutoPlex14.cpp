...
void AutoPlex14::wipeDisplay() { // Used internally to turn the display off without clearing the buffer
  ...
  for (uint8_t i = 0; i < 15; i++) { fastWrite(segmentPinsClass[i], segmentOff); }
}
...
bool AutoPlex14::begin(uint8_t displayType, uint8_t digits, uint8_t digitPins[], uint8_t segmentPins[], bool multiplexing) { // Initializes the display; returns true if successful
  ...
  for (uint8_t i = 0; i < 15; i++) { if (digitalPinToPort(segmentPins[i]) == NOT_A_PIN) { return false; } } // Validate segment pins
...
void AutoPlex14::multiplex() { // Render the buffer onto the screen
  ...
  if (strlen32(buffer) > bufferPos && buffer[bufferPos + 1] == U'.') { fastWrite(segmentPinsClass[14], segmentOn); } // Turn on this digit's DP if the next character is a decimal
  ...
  for (int8_t i = 13; i >= 0; i--) {
    fastWrite(segmentPinsClass[13 - i], bitRead(bitmap, i) ? segmentOn : segmentOff);
  }
